...
export type ParseDateResult = { ok: true; isoUtc: string } | { ok: false; reason: string };
...
export function parseDateInput(input: string, timezone: string, now = new Date()): ParseDateResult {
  ...
  for (const format of EXPLICIT_FORMATS) {
    ...
    // Luxon normalizes nonexistent DST wall times; round-tripping rejects that silent shift.
    if (candidate.isValid && candidate.toFormat(format) === value) {
      parsed = candidate;
      break;
    }
  }
  ...
  if (!parsed) {
    ...
    if (match) {
      ...
      if (hour <= 23 && minute <= 59 && (!meridiem || Number(match[1]) <= 12)) {
        const candidate = DateTime.fromJSDate(now).setZone(timezone).plus({ days: 1 })
  ...
  if (!parsed) {
    ...
    if (match) {
      ...
      if (amount > 0) {
        ...
        parsed = unit.startsWith("m") ? base.plus({ minutes: amount })
          : unit.startsWith("h") ? base.plus({ hours: amount }) : base.plus({ days: amount });
      }
    }
  }

  if (!parsed || !parsed.isValid) {
    return { ok: false, reason: "I could not read that time. Try 2026-06-19 07:00, 19/06/2026 07:00, tomorrow 7am, or in 2 hours." };
  }
  if (parsed.toMillis() <= now.getTime()) {
    return { ok: false, reason: "Please choose a time in the future." };
  }
  return { ok: true, isoUtc: parsed.toUTC().toISO() as string };
}

export function formatDate(isoUtc: string, timezone: string): string {
  return DateTime.fromISO(isoUtc, { zone: "utc" }).setZone(timezone).toFormat("yyyy-MM-dd HH:mm ZZZZ");
}

export function presetTime(
  preset: "10m" | "30m" | "1h" | "tomorrow7" | "tomorrow8" | "next20",
  timezone: string,
  now = new Date(),
): string {
  const local = DateTime.fromJSDate(now).setZone(timezone);
  if (preset === "10m") return local.plus({ minutes: 10 }).toUTC().toISO() as string;
  if (preset === "30m") return local.plus({ minutes: 30 }).toUTC().toISO() as string;
  if (preset === "1h") return local.plus({ hours: 1 }).toUTC().toISO() as string;
  if (preset === "tomorrow7" || preset === "tomorrow8") {
    return local.plus({ days: 1 }).set({ hour: preset === "tomorrow7" ? 7 : 8, minute: 0, second: 0, millisecond: 0 })
      .toUTC().toISO() as string;
  }
  let next = local.set({ hour: 20, minute: 0, second: 0, millisecond: 0 });
  if (next.toMillis() <= local.toMillis()) next = next.plus({ days: 1 });
  return next.toUTC().toISO() as string;
}

export function nextTwentyLabel(timezone: string, now = new Date()): string {
  const local = DateTime.fromJSDate(now).setZone(timezone);
  return local.hour < 20 ? "Tonight 20:00" : "Tomorrow 20:00";
}
