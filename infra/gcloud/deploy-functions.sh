...
npm test
npm run build
npx firebase deploy --project "$PROJECT_ID" --account "$FIREBASE_ACCOUNT" --only functions:taskHandler
...
npx firebase deploy --project "$PROJECT_ID" --account "$FIREBASE_ACCOUNT" --only firestore,functions
